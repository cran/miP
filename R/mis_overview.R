mis_overview <-
function(ds,srt=0,pos=-3,percent=FALSE)
{
abstand=pos
...
if(srt==0)
{p = barplot(mis_table/tief*100,xlab = "variables",ylab="percent", main=paste("Total data missing:",round(mis_tot_per*100,2),"%"), col = color,legend=FALSE,axisnames=TRUE)
if(percent==TRUE){
...
{p = barplot(mis_table/tief*100,xlab = "variables",ylab="percent", main=paste("Total data missing:",round(mis_tot_per*100,2),"%"), col = color,legend=FALSE,axisnames=TRUE,names.arg=rep("",breit))
if(percent==TRUE)
{
text(p, rep(100+abstand,breit), paste(round(mis_data/tief*100,2),"%"), xpd=TRUE)
}
